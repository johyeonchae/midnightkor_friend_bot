...
# 지갑 주소 미등록자 안내 링크
WALLET_FORM_URL = "https://docs.google.com/forms/d/e/1FAIpQLSfRGon2D9kpELh4kfcLZ5vsZ8LplP1J84_OWUVn6w5U-7RuQw/viewform"
...
def format_settlement_msg(data: dict) -> str:
    ...
    if s1p.get('total', 0) > 0 or s1_amt > 0:
        ...
        if s1p.get('holder'):
            details.append(f"💎 홀더 인증: {s1p['holder']:,}pt")
        if s1p.get('kol_holder'):
            details.append(f"📣 KOL 홀더 인증: {s1p['kol_holder']:,}pt")
        if details:
    ...
    if addr and chain and chain != '주소없음':
        ...
        msg += f'🔗 <a href="{WALLET_FORM_URL}">포인트 캠페인 시즌1 문의사항 접수</a>\n\n'
